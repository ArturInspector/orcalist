...
async def api_listing(req: ListingReq):
    ...
    try:
        ...
        if not wallet or not amount or amount <= 0:
            logger.warning(f"Missing required fields: wallet={wallet is not None}, amount={amount}")
            raise HTTPException(status_code=400, detail="Invalid request")
        ...
        try:
            ...
            logger.warning(
                f"Invalid wallet address: wallet={safe_wallet_log(wallet) if wallet else 'None'}, "
                f"wallet_hash={hash_wallet(wallet) if wallet else 'none'}, "
                f"length={len(wallet) if wallet else 0}, error_type={type(e).__name__}"
            )
            raise HTTPException(status_code=400, detail="Invalid request")
